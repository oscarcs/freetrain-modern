...
namespace freetrain.world.development
{
	...
	public class DevelopmentAlgorithm : ULVFactory {
		...
		static internal protected int FindFirstIndexOf(ArrayList array, long price ){
			int n = array.Count>>1;
			int p = n;
			do{
				n=(n+1)>>1;
				long ppa = (long)((IEntityBuilder)array[p]).pricePerArea;
				if(ppa>=price)
					p = Math.Max(0,p-n);
				else
					p = Math.Min(array.Count-1,p+n);
			}while(n>1);
			if(price > (long)((IEntityBuilder)array[p]).pricePerArea )
				p++;
...
//			Debug.WriteLine(string.Format("which is {0} -> {1} <- {2}",((IEntityBuilder)array[Math.Max(0,p-1)]).pricePerArea,((IEntityBuilder)array[p]).pricePerArea,((IEntityBuilder)array[Math.Min(array.Count-1,p+1)]).pricePerArea));
		...
		static internal protected int FindLastIndexOf(ArrayList array, long price ){
			int n = array.Count>>1;
			int p = n;
			do{
				n=(n+1)>>1;
			...
			if(price == (long)((IEntityBuilder)array[p1]).pricePerArea )
				p = p1;			
...
#region riorio 2010.04.17 Fixed
		public void Update(){
		...
		public void Process(){
			...
			switch(phase){
				...
				case Phase.SelectStruct1: {
					...
					long minVal = (long)(World.world.landValue[scaning]*F_LandPriceScale);
					long maxVal = Math.Max(minVal+15,(int)(minVal*1.2));
                    ...
                    minVal *= 10000;  maxVal *= 10000; //2010.05.04 riorio add for VER2 plugins price
...
//				case Phase.Build:{
		...
		protected bool IsReplaceable(Entity e, long comPrice){
			comPrice = (long)(comPrice*F_ReplacePriceFactor);
