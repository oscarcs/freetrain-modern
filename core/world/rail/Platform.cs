...
//		protected Platform( Location loc, Direction d, int len ) {
		protected Platform( Location loc, Direction d, int len, bool withRoof ) {
			this.location = loc;
			this.direction = d;
			this.length = len;
			this.name = string.Format("ホーム{0,2:d}",iota++);
			this.bellSound = DepartureBellContribution.DEFAULT;
			repeatHandler = World.world.clock.registerRepeated( new ClockHandler(onClockPerDay), TimeLength.ONEDAY );
		...
		/// <summary> platform repeat clock handler </summary>
		...
		public void onClockPerDay() {
			// charge the cost
			AccountManager.theInstance.spend( 180000*length, AccountGenre.RAIL_SERVICE );
		}
