// 2009.05.14 Yz プラグインDLLが指定場所にない場合はプラグインディレクトリ配下を検索するように修正
using System;
	...
	public class PluginUtil
	{
		...
		public static Type loadTypeFromManifest( XmlElement e ) {
			...
			if( e.Attributes["codebase"]==null ) {
                ...
                if (System.IO.File.Exists(codeBase.LocalPath) == true) {        // codebaseで指定したDLLが存在する場合    
    				a = Assembly.LoadFrom( codeBase.LocalPath );                // 指定したパスからロード
                } else {                                                        // 存在しない場合
                                                                                // プラグインディレクトリ配下を検索
                    string[] files = System.IO.Directory.GetFiles(PluginManager.getDefaultPluginDirectory(), System.IO.Path.GetFileName(codeBase.LocalPath), System.IO.SearchOption.AllDirectories);
                    a = Assembly.LoadFrom(files[0]);                            // 最初に見つかった場所からロード
                }
			}
