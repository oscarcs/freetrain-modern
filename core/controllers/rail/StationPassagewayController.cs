...
{/// <summary>
	/// StationPassagewayController の概要の説明です。
	/// </summary>
	public class StationPassagewayController : AbstractControllerImpl, MapOverlay, LocationDisambiguator
	{
		...
		private StationPassagewayController() {
			...
			typeCombo.Items.Add("木造（茶）");
			typeCombo.Items.Add("木造（緑）");
		}
		...
		public void drawVoxel( QuarterViewDrawer view, DrawContextEx canvas, Location loc, Point pt ) {
			...
			if( tag!=null && (bool)tag && loc.inBetween( anchor, location ) ) {
                switch (typeCombo.SelectedIndex) {
                    case 0:
                        PassagewayRail.getFloatingSprite( anchor.getDirectionTo(location), ThinPlatform.PassagewayRailType.pwrtConcrete )
					        .drawAlpha( canvas.surface, pt );
                        break;
                    case 1:
                        PassagewayRail.getFloatingSprite( anchor.getDirectionTo(location), ThinPlatform.PassagewayRailType.pwrtWoodenBrown )
					        .drawAlpha( canvas.surface, pt );
                        break;
                    case 2:
                        PassagewayRail.getFloatingSprite( anchor.getDirectionTo(location), ThinPlatform.PassagewayRailType.pwrtWoodenGreen )
					        .drawAlpha( canvas.surface, pt );
                        break;
                    default:
                        PassagewayRail.getFloatingSprite( anchor.getDirectionTo(location), ThinPlatform.PassagewayRailType.pwrtConcrete )
		...
		private void buildPassageway( Location loc1, Location loc2 ) {
			Direction dd = loc1.getDirectionTo(loc2);	// direction
			Debug.Assert(dd.isSharp);
					...
					if(tv.railRoad is ThinPlatform.RailRoadImpl) {
						...
						if( rr.outlook is ThinPlatform.PassagewayPlatform ) {
							...
							if( ppp.hasBridge ) {
                                ...
                                switch (typeCombo.SelectedIndex) {
                                    case 0: ((ThinPlatform.PassagewayPlatform)rr.outlook).passagewayRailType = ThinPlatform.PassagewayRailType.pwrtConcrete;
                                            break;
                                    case 1: ((ThinPlatform.PassagewayPlatform)rr.outlook).passagewayRailType = ThinPlatform.PassagewayRailType.pwrtWoodenBrown;
                                            break;
                                    case 2: ((ThinPlatform.PassagewayPlatform)rr.outlook).passagewayRailType = ThinPlatform.PassagewayRailType.pwrtWoodenGreen;
                                            break;
                                    default: ((ThinPlatform.PassagewayPlatform)rr.outlook).passagewayRailType = ThinPlatform.PassagewayRailType.pwrtConcrete;
                                             break;
                                }
							}
						} else {
		...
		private bool createStair( Location loc, bool test) {
			...
			if(nrr!=null && nrr.outlook is ThinPlatform.PassagewayPlatform) {
                ...
                switch (typeCombo.SelectedIndex) {
                    case 0: ((ThinPlatform.StairPlatform)rr.outlook).passagewayRailType = ThinPlatform.PassagewayRailType.pwrtConcrete;
                            break;
                    case 1: ((ThinPlatform.StairPlatform)rr.outlook).passagewayRailType = ThinPlatform.PassagewayRailType.pwrtWoodenBrown;
                            break;
                    case 2: ((ThinPlatform.StairPlatform)rr.outlook).passagewayRailType = ThinPlatform.PassagewayRailType.pwrtWoodenGreen;
                            break;
                    default: ((ThinPlatform.StairPlatform)rr.outlook).passagewayRailType = ThinPlatform.PassagewayRailType.pwrtConcrete;
                             break;
                }
