...
namespace freetrain.finance.stock
{
	...
	public class Economy : ITrendTarget
	{
		...
		private ClockHandler repeatHandlerMajorUpdateEvent = null;
		private ClockHandler repeatHandlerMinorUpdateEvent = null;
		private int phase1;
		private int phase2;
		...
		private void setClockHandlers() {
			...
			if(MajorUpdateEvent != null ) 
				clock.unregister( repeatHandlerMajorUpdateEvent );
			MajorUpdateEvent = new ClockHandler(onMajorUpdate);
			long major = MajorUpdateStartingHour;
			while( major<today_h )
				major += MajorUpdateIntervalHours;
			repeatHandlerMajorUpdateEvent = clock.registerRepeated(MajorUpdateEvent, 
			...
			if(MinorUpdateEvent != null ) 
				clock.unregister( repeatHandlerMinorUpdateEvent );
			MinorUpdateEvent = new ClockHandler(onMinorUpdate);
			long minor = MinorUpdateStartingHour;
			while( minor<today_h )
				minor += MinorUpdateIntervalHours;
			repeatHandlerMinorUpdateEvent = clock.registerRepeated(MinorUpdateEvent, 
