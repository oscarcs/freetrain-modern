...
namespace freetrain.world.structs
{
	...
	/// </summary>
	[Serializable]
	public class SpecialStationListenerImpl : rail.SpecialStationListener
	{
		...
		public SpecialStationListenerImpl( Population pop, Location loc ) {
			this.population = pop;
			this.location = loc;
		...
		public void onRemoved() {
			...
			{
				station.speciallisteners.remove(this);
			}
			stations.Clear();
		}
		...
		public bool advertiseStation( Station s ) {
			...
			else	
			{
				...
				if(remove!=null)
				{
					remove.speciallisteners.remove(this);
		...
		private void attachToStation() {
			foreach( Station s in World.world.stations ) {
				if( !s.withinSpecialReach(location) )
					continue;
				advertiseStation(s);					
			}
		}
