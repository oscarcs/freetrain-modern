...
//						Point pt = fromHVZToClient(h,v,z);
		...
		private void updateScreen() 
		{
            ...
            if( Core.options.drawStationNames ) 
			{
                ...
                drawBrush2 = new SolidBrush( Core.fontoptions.colorStationNames );
                foreach( freetrain.world.rail.Station st in world.stations ) 
				{
					...
					SizeF sz = graphics.MeasureString( st.name, Core.fontoptions.fontStationNames );
					pt.X -= (int)sz.Width/2;

                    graphics.DrawString(st.name, Core.fontoptions.fontStationNames, drawBrush1, pt.X + 1, pt.Y + 1);
                    graphics.DrawString(st.name, Core.fontoptions.fontStationNames, drawBrush2, pt.X, pt.Y);
				}
			}

			if( Core.options.drawTrainNames ) 
			{
				searchTrain( World.world.rootTrainGroup );
			}
        ...
        private void searchTrain( TrainGroup group ){
            foreach( TrainItem item in group.items ) {
                if( item is TrainGroup ) {
                    searchTrain( (TrainGroup)item );
                } else {
                    Train tr = (Train)item;
                    if( tr.isPlaced )
                        drawTrainName( tr );
        ...
        private void drawTrainName( Train tr )
        {
            if( tr.head.state.asInside()==null ) return;
          
			// REVISIT: I don't want these code inside this method.
			//  it needs to be extensible.
			Graphics graphics = drawContext.graphics;
            drawBrush2 = new SolidBrush( Core.fontoptions.colorTrainNames );

		    Point pt = fromXYZToClient( tr.head.state.asInside().location );
		    pt.Y -= 8;	// display the string above the train

		    SizeF sz = graphics.MeasureString( tr.name, Core.fontoptions.fontTrainNames );
		    pt.X -= (int)sz.Width/2;

            graphics.DrawString(tr.name, Core.fontoptions.fontTrainNames, drawBrush1, pt.X + 1, pt.Y + 1);
            graphics.DrawString(tr.name, Core.fontoptions.fontTrainNames, drawBrush2, pt.X, pt.Y);
        }
